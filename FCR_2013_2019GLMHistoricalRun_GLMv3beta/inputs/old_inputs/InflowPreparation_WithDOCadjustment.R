...
weir_inflow <- alldata %>% 
  ...
  mutate(OGM_pon = (1/6)*(TN_ugL-(NIT_amm+NIT_nit))) %>% #determined by subtraction
...
ghg_inflows <- read.csv("Dissolved_CO2_CH4_Virginia_Reservoirs.csv", header=T) %>%
  ...
  select(DateTime, Site, ch4_umolL) %>%
  group_by(DateTime, Site) %>% 
  summarise(ch4_umolL=mean(ch4_umolL)) %>%
  rename(CAR_ch4 = ch4_umolL) %>%
  pivot_wider(names_from = Site, values_from = CAR_ch4, names_prefix="x.") %>%
  na.omit() %>%
...
wetland_inflow <- data1 %>% #using weir chemistry in workspace for multiplying ratios
  ...
  mutate(OGM_poc = 0.1*(OGM_doc+OGM_docr)) %>% #assuming that 10% of DOC is POC (Wetzel page 755)
  mutate(OGM_don = (5/6)*(TN_ugL-(NIT_amm+NIT_nit))*0.10) %>% #DON is ~5x greater than PON (Wetzel page 220)
  mutate(OGM_donr = (5/6)*(TN_ugL-(NIT_amm+NIT_nit))*0.90) %>% #keeping all recalcitrant dissolved organic pools at 90%
  mutate(OGM_pon = (1/6)*(TN_ugL-(NIT_amm+NIT_nit))) %>% #by subtraction
  mutate(OGM_dop = 0.3*(TP_ugL-PHS_frp)*0.1) %>% #Wetzel page 241, 70% of total organic P = particulate organic; 30% = dissolved organic P
  mutate(OGM_dopr = 0.3*(TP_ugL-PHS_frp)*0.9) %>% #keeping all recalcitrant dissolved organic pools at 90%
  mutate(OGM_pop = 10*TP_ugL) %>% #In lieu of having the adsorbed P pool activated in the model, need to have higher complexed P
  mutate(CAR_dic = DIC_mgL*1000*(1/52.515)) %>% #Long-term avg pH of FCR is 6.5, at which point CO2/HCO3 is about 50-50
#given this disparity, using a 50-50 weighted molecular weight (44.01 g/mol and 61.02 g/mol, respectively)
...
#now need to get temp for the wetland inflow; setting to 0.1 m water temp at site 50 using CTD
...
#get water temp for wetland inflow file, setting Dec 31 of each year to 4oC in lieu of CTD data for interpolation
