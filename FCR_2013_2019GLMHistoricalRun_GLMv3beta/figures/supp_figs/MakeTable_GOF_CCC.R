...
comb_Totals_cal <- left_join(obs_Totals_cal,mod_totals2_cal,by=c("DateTime","Depth")) %>% 
  ...
  summarise(TN_obs = mean(TN_obs), 
            TN_mod = mean(TN_mod)) %>% 
  ungroup()
all_gof_cal$TN[1:21] <- gof(comb_Totals$TN_mod,comb_Totals$TN_obs,na.rm=TRUE,do.spearman = TRUE)
...
comb_Totals_val <- left_join(obs_Totals_val,mod_totals2_val,by=c("DateTime","Depth")) %>% 
  ...
  summarise(TN_obs = mean(TN_obs), 
            TN_mod = mean(TN_mod)) %>% 
  ungroup()
all_gof_val$TN[1:21] <- gof(comb_Totals$TN_mod,comb_Totals$TN_obs,na.rm=TRUE,do.spearman = TRUE)
...
comb_Totals_cal <- left_join(obs_Totals_cal,mod_totals2_cal,by=c("DateTime","Depth")) %>% 
  ...
  ungroup()
all_gof_cal$TN[1:21] <- gof(comb_Totals$TN_mod,comb_Totals$TN_obs,na.rm=TRUE,do.spearman = TRUE)
all_gof_cal$TP[1:21] <- gof(comb_Totals$TP_mod,comb_Totals$TP_obs,na.rm=TRUE,do.spearman = TRUE)

comb_Totals_cal_rank <- comb_Totals_cal %>% 
  mutate(rank_obsTN = rank(TN_obs),
         rank_modTN = rank(TN_mod),
         rank_obsTP = rank(TP_obs),
         rank_modTP = rank(TP_mod)) 
all_gof_cal$TN[22] <- summary(lm(comb_Totals_cal_rank$rank_obsTN ~ comb_Totals_cal_rank$rank_modTN))$r.squared
...
comb_Totals_val <- left_join(obs_Totals_val,mod_totals2_val,by=c("DateTime","Depth")) %>% 
  ...
  ungroup()
all_gof_val$TN[1:21] <- gof(comb_Totals$TN_mod,comb_Totals$TN_obs,na.rm=TRUE,do.spearman = TRUE)
all_gof_val$TP[1:21] <- gof(comb_Totals$TP_mod,comb_Totals$TP_obs,na.rm=TRUE,do.spearman = TRUE)
